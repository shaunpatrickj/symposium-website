...
export default function EventCard({ event }: EventCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`group relative h-full bg-gray-900/40 border border-gray-700/50 p-6 backdrop-blur-sm hover:border-cyan-400/50 transition-colors ${isPaperPresentation ? '' : 'overflow-hidden'
        }`}
    >
      {/* Container for background elements that need clipping */}
      <div className={`absolute inset-0 ${isPaperPresentation ? 'overflow-hidden rounded-[inherit]' : ''}`}>
